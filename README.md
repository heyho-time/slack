...
- eslint 설정 파일
- 코드 검사 도구, 직접 설정하면 팀원간 의견 충돌이 있으니 prettier에 위임
- npm i -D eslint

3. .prettierrc

- prettier 설정 파일 (코드 정렬 도구)
...
- 웹팩 설정
- ts, css, json, 최신 문법 js 파일들을 하나로 합쳐줌. 번들러.
...
### npm run build 해줘야 반영됨 -> 불편 -> hot reloading 세팅
...
- 3초 룰 기억하자!
  수백 페이지를 한 JS 덩어리로 만들면 성능이 당근 떨어지니..
  1. 페이지단위로 분리하라.
  2. SSR이 필요없는 애들을 분리해라.
